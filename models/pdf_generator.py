...
class PDFGenerator:
    ...
    def __init__(self, cv, output_path, sidebar_colour="#2C3E50"):
        """
        Constructor — takes a CV object, output path and sidebar colour.
        """
    ...
    def generate(self):
        ...
        for item in contact_items:
            if item:
                wrapped = simpleSplit(item, "Helvetica", 8, 5.2 * cm)
                for line in wrapped:
        ...
        wrapped_summary = simpleSplit(sections.get("summary", ""), "Helvetica", 9, main_width)
        ...
        for edu in sections.get("education", []):
            edu = edu.strip()
            if edu:
                wrapped = simpleSplit(edu, "Helvetica", 9, main_width)
                for line in wrapped:
                    c.drawString(main_x, main_y, line)
                    main_y -= 0.4 * cm
                main_y -= 0.2 * cm
        ...
        for exp in sections.get("experience", []):
            exp = exp.strip()
            if exp:
                wrapped = simpleSplit(exp, "Helvetica", 9, main_width)
                for line in wrapped:
                    c.drawString(main_x, main_y, line)
                    main_y -= 0.4 * cm
                main_y -= 0.2 * cm
